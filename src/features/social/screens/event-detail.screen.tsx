...
import { Avatar, Box, Button, Divider, FakeInput, Input, LoadingPage, ThemedText, Touchable } from '@src/shared/components'
...
import { EventPhotoPicker } from '../components/event-photo-picker.component'
...
type EventDetailScreenProps = NativeStackScreenProps<ModalNavigatorParamsList, 'EventDetailScreen'>
...
export const EventDetailScreen: React.FC<EventDetailScreenProps> = ({ route, navigation }) => {
  ...
  useEffect(() => () => setPickerPlace(null), [setPickerPlace])
  ...
  const { mutate: updateEvent, isPending: isUpdatingEvent } = useMutation({
    ...
    onSuccess: ({ data }) => {
      ...
      queryClient.setQueryData(['eventDetail', eventId], data)
  ...
  const handleOpenPlaceSearch = () => {
    ...
    navigation.navigate('EventPlaceSearchScreen')
  }
  ...
  const handleOpenPlace = () => {
    ...
    navigation.navigate('PlacesDetailsScreen', { placeId: pickerPlace.id })
  }
